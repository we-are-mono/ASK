...
cdx/control_pppoe.h:67-74 (struct _tPPPoECommand). The fcode encodes the
...
(cdx/control_pppoe.c:321-405). Length validation is enforced by
...
CMD_PPPOE_ENTRY = 0x0601  # cdx/cdx_cmdhandler.h:167
...
PPPOE_LEAK_FILTER = [
    ...
    "PPPoE_Handle_Relay_Entry",  # relay REGISTER/DEREGISTER
    "pppoe_entry_handle",        # dispatch wrapper
    "pppoe_relay_entry_handle",  # dispatch wrapper
...
def pppoe_cmd(
    ...
    struct PPPoECommand (cdx/control_pppoe.h:67-74) is not __packed; the
    ...
    cdx/control_pppoe.c:317, so callers pass it host-endian here.
    """
    return (
