...
- [~] **A3e. `dpa_add_eth_if` — `err_ret*` cascade has three explicit TODO gaps.**
  ...
  _Partial done:_ added `dpa_remove_port_from_list(iface_info)` helper and wired it into `err_ret3` — that was the simplest of the four gaps and the only one with a trivially invertible operation (the port list is a straight singly-linked list protected by `dpa_devlist_lock`). The three remaining TODOs (`err_ret7` disable-CEETM, `err_ret6` reset-bman-discard-mask, `err_ret5` remove-ff-policer) need reverse helpers that don't exist in the codebase yet — writing them requires understanding each subsystem's teardown sequence (CEETM channel tear-down in `cdx_ceetm_app.c`; BMan per-port discard-mask state; policer profile de-allocation). Forcing a half-baked implementation would be worse than leaving the TODO explicit; replaced each stub comment with a clearer note about what's leaked and where the matching reverse helper would need to live. Tracked as residual work._
