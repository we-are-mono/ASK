...
- [ ] **A9. Tunnel offload — premise inverted by on-DUT tracing (2026-08-09): RX decap WORKS; it's TX encap that never offloads.**
  ...
  **Remaining work:** (1) root-cause the tunnel-encap key/lookup mismatch (printk in fill_key_info/create_tunnel_insert_hm on both insert and a hexdump of the eth4 EHASH bucket, or FMAN CC dump via sysfs); (2) rewrite the decap test inner-keyed + un-xfail; (3) fix the TX test's oracle to a counter-based one; (4) audit occupancy-based golden tests; (5) then the `ip6_tunnel.c` stamp + 4o6 test per the decision below.
