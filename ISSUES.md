...
- [ ] **A9. Tunnel offload — premise inverted by on-DUT tracing (2026-08-09): RX decap WORKS; it's TX encap that never offloads.**
  ...
  **Remaining work:** (1) FMAN-side bucket/ucode comparison per above; (2) rewrite the decap test inner-keyed + un-xfail; (3) fix the TX test's oracle to a counter-based one; (4) audit occupancy-based golden tests; (5) then the `ip6_tunnel.c` stamp + 4o6 test per the decision below.
