...
- [ ] **A9. Tunnel offload — premise inverted by on-DUT tracing (2026-08-09): RX decap WORKS; it's TX encap that never offloads.**
  ...
  **Remaining work:** (1) find why eth4-ingress traffic misses the encap entry and fix TX offload; (2) rewrite the decap test inner-keyed + un-xfail; (3) fix the TX test's oracle to a counter-based one; (4) fqid_stats zero-counter investigation; (5) then the `ip6_tunnel.c` stamp + 4o6 test per the decision below.
