...
- [ ] **A14. H2 IPsec key-zeroing regression test requires kernel-side observability probe.**
  No user-space surface tells us whether `kfree_sensitive` actually zeroed the key bytes — kmemleak reports references not contents, KASAN poison is internal. Parked at [tools/tests/test_ipsec_key_zeroing.py](tools/tests/test_ipsec_key_zeroing.py) with `pytest.skip` until a debugfs probe surfaces freed-but-not-yet-reused slab contents.

- [ ] **A15. cmm has no incoming xfrm netlink subscription.**
  Only outbound `XFRM_MSG_EXPIRE` send in [cmm/src/keytrack.c:1613-1661](cmm/src/keytrack.c#L1613-L1661); no `RTNLGRP`-style subscription for `XFRM_MSG_NEWSA`/`NEWPOLICY`/`DELSA`/`DELPOLICY`. Slice-2 IPsec E2E fixture must drive `ip xfrm state add/del` AND `CMD_IPSEC_SA_*` in parallel; cmm cannot auto-bridge xfrm events to FCI. Design finding, not a bug — implementation would mirror the rtnetlink/nfct watchers but isn't required for offload to function.

- [ ] **A16. NAT-T fast-path push silently discards classification-table-entry failure.**
  [cdx/control_ipsec.c:720](cdx/control_ipsec.c#L720) — `ipsec_push_sa_to_fast_path` calls `cdx_ipsec_process_udp_classification_table_entry(sa)` and discards the return value. `cdx_ipsec_process_udp_classification_table_entry` returns `FAILURE` on bounds-check fire (the H5 site at line 2369) and on `dpa_get_iface_info_by_ipaddress` lookup failure, but the caller can't tell — `reply_rc` to user-space stays NO_ERR. Effective oracle for any failure here is splat_window/KASAN. **Fix:** propagate the return up through `ipsec_push_sa_to_fast_path` to `IPsec_handle_SA_SET_STATE` so user-space sees a real error code.

- [ ] **A17. `IPsec_handle_SA_SET_KEYS` dereferences `sa` before NULL check.**
  [cdx/control_ipsec.c:617](cdx/control_ipsec.c#L617) writes `sa->pSec_sa_context->auth_data.auth_type = 0;` immediately after `sa = M_ipsec_sa_cache_lookup_by_h(cmd.sagd);` and BEFORE `if (sa == NULL) return ERR_SA_UNKNOWN;` at line 619. Sending `CMD_IPSEC_SA_SET_KEYS` for a `sagd` that doesn't exist NULL-derefs. Reachable from any FCI sender (cmm, askd-agent, test harness) without elevated privilege beyond what FCI dispatch already requires. **Fix:** move the assignment after the NULL check.

- [ ] **A18. NAT-T fast-path push NULL-derefs `sa->ct` on synthetic-SA install failure.**
  [cdx/cdx_dpa_ipsec.c:2384-2389](cdx/cdx_dpa_ipsec.c#L2384-L2389) — `cdx_ipsec_process_udp_classification_table_entry`'s else-branch calls `cdx_ipsec_add_classification_table_entry(sa)`, ignores its return, then unconditionally writes `sa->ct->natt_in_refcnt = 1;`. When the inner call fails (e.g. `dpa_get_iface_info_by_ipaddress` lookup fail on a synthetic SA without a real iface) `sa->ct` stays NULL → `0x21` offset deref → kernel oops. Reproduced on any inbound NAT-T SA installed against a destination IP not bound to a netdev. Surfaced while writing slice-1 IPsec tests; blocks H3/H5 regression tests at [tools/tests/test_ipsec_dma_balance.py](tools/tests/test_ipsec_dma_balance.py) and [tools/tests/test_ipsec_natt_spi_bounds.py](tools/tests/test_ipsec_natt_spi_bounds.py) which are parked until this is fixed. **Fix:** check `cdx_ipsec_add_classification_table_entry`'s return; on failure either bail or check `sa->ct` before the refcnt writes.

- [x] **A19. IPsec SA install/release leak triad.**
  Three asymmetric-cleanup leaks: (1) [cdx/procfs.c:158](cdx/procfs.c#L158) `cdx_create_dir_in_procfs` leaked the kmalloc'd wrapper struct on `proc_mkdir` NULL return (failslab-only path); (2) [cdx/dpa_ipsec.c](cdx/dpa_ipsec.c) `cdx_dpa_ipsecsa_release` leaked the `cdx_proc_dir_entry_t` wrapper struct because `proc_remove` only frees the kernel's `proc_dir_entry`, not the CDX-private wrapper (8 B per CREATE+DELETE); (3) [cdx/dpa_ipsec.c](cdx/dpa_ipsec.c) `cdx_dpa_ipsecsa_release` leaked `sainfo->shdesc_mem` (~512 B per CREATE+DELETE) — `create_ipsec_fqs` correctly frees it on partial-init failure but the matching free was missing on the success-then-release path. Surfaced by [tools/tests/test_ipsec_failslab.py](tools/tests/test_ipsec_failslab.py) under failslab + kmemleak. _Fixed: pending commit_ — three matching `kfree`s; verified by 5 CREATE+DELETE → 0 ipsec leaks under strict kmemleak.
