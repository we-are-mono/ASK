...
- **A28.** xfrm_input.c inbound-offload submit passes x->handle to SEC gated only by x->offloaded, *before* the XFRM_STATE_VALID check — and x->offloaded is never cleared on SA delete, so a dead-but-still-offloaded SA can submit a packet with a stale handle. Teardown-ordering, pre-existing in patch 040; surfaced while auditing A27. Open.
