...
- **A30.** ctnetlink_change_permanent() short-circuited the entire conntrack update whenever CTA_STATUS carried IPS_PERMANENT (the bit's *value*), so an ordinary update echoing an already-set permanent bit silently dropped the bundled CTA_MARK/CTA_NAT/CTA_PROTOINFO/CTA_QOSCONNMARK changes. Gate on the IPS_PERMANENT *delta* instead: only a real flip is a pin/unpin (applied + short-circuited); a no-flip returns -1 so ctnetlink_change_conntrack() applies the rest (re-applying IPS_PERMANENT via nf_ct_change_status_common is idempotent). Pin switched to atomic set_bit to match unpin's clear_bit (status is also set locklessly by the datapath). A no-flip update no longer consults CTA_ID — consistent with the normal update path. Patch 050.
- **A31.** ctnetlink_change_permanent()'s unpin trigger is "CTA_STATUS present but omitting IPS_PERMANENT on a pinned ct" (the bit going 1->0): a routine update that merely omits the bit silently unpins the ct and short-circuits (dropping bundled attrs). Pre-existing (A30 preserves it per the "don't break unpin" constraint); benign as long as the controller always echoes the full status of a pinned ct. Open (defense-in-depth: gate unpin on an explicit signal rather than bit-absence).
