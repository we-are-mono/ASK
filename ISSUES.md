...
- [ ] **A9. Tunnel offload — premise inverted by on-DUT tracing (2026-08-09): RX decap WORKS; it's TX encap that never offloads.**
  ...
  **Remaining work:** (1) ~~FMAN-side comparison~~ done — encap blocked in ucode INSERT_L3_HDR (above); (2)+(5) real-path decap tests written ([test_tunnel_decap_offload.py](tools/tests/test_tunnel_decap_offload.py), 6o4 + 4o6) and the synthetic outer-keyed test retired — **the tests encode the right counter oracle and both paths are hand-verified passing, but the automated runs need a shared-fixture timing fix**: `ipv6_topology` adds loki's `fc00:dead::2` without `nodad`, so it's DAD-tentative for ~1.5 s and the decap flow can't start until it settles; a readiness poll was added test-side but the runs still don't come up cleanly on this lab setup — next session either add `nodad` (or a DAD wait) to the `ipv6_topology` fixture in `_topology.py`, or stage the echo server earlier; (3) TX test reframed with a kernel-sit-tx tripwire (done, [test_tunnel_tx_offload.py](tools/tests/test_tunnel_tx_offload.py)); (4) occupancy-based golden audit still open. Debug printks have been stripped from cdx (reverted clean).
