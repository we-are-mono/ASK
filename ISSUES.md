...
- [ ] **A24. SEC GCM offload — wrong descriptor share-mode caused ~86 % AEAD ICV-fail at line-rate. Functional fix landed; wire-seq-uniqueness follow-up open as A24a.**
  Diagnosed and partially fixed 2026-05-05 by side-by-side cipher comparison against the manual `ip xfrm` bring-ups in [IPSEC.md](IPSEC.md). All ciphers engage the cdx hardware encap path (DUT `xfrm_output` doesn't fire, DUT xfrm SA byte counters stay at 0, all wire frames are ESP).

  | Cipher | UDP @ 1 Gbit/s, 5 s | seq dupes / 500 | Vision `XfrmInStateProtoError` Δ |
  | ------ | ------------------- | --------------- | -------------------------------- |
  | AES-128-CBC + HMAC-SHA-256 | 999 Mbit/s, 0.06 % loss | 0 | 0 (noise) |
  | AES-128-CCM-ICV16 | 1.00 Gbit/s, 0.005 % loss | 0 | 0 |
  | AES-128-GCM-ICV16 (before fix) | 140 Mbit/s, **86 % loss** | 0-4 sporadic | **+384 000 in 5 s** |
  | AES-128-GCM-ICV16 (after fix) | **999 Mbit/s, 0.087 % loss** | **106 (~21 %) — A24a** | **0** |
  | AES-128-GCM-ICV16 TCP (after) | **4.98 Gbit/s, 2541 retx (15 s)** | n/a | 0 |

  **Root cause**: cdx initialised the GCM shared descriptor with `HDR_SHARE_SERIAL` (cdx/cdx_dpa_ipsec.c, encap+decap). Under that mode, multiple SEC DECOs work on the same descriptor concurrently for ops on a single SA. SEC parallelises the GCM AEAD path (GHASH is GF(2¹²⁸)-multiply-based and inherently parallelisable — unlike CBC-MAC for CCM or HMAC for CBC, both of which serialise naturally) — and the cross-DECO contention on per-SA GHASH state writes packets where the AAD-SEQ used in the ICV computation drifts from the SEQ written to the wire ESP header. Peer reconstructs AAD with the wire SEQ, computes a different ICV, drops the packet. NXP's shipped reference cdx (`/home/tzaman/Mono/ASK-NXP/cdx/cdx-5.03.1/cdx_dpa_ipsec.c:751`) has the same defect — they ship `HDR_SHARE_SERIAL` for all ciphers including GCM. Likely never noticed because their test peer also disabled anti-replay and they never measured ICV-fail rates.

  **Fix landed**: gate `HDR_SHARE_NEVER` on for GCM/GMAC cipher types and keep `HDR_SHARE_SERIAL` for CBC/CCM/CTR (which work). Single edit in [cdx/cdx_dpa_ipsec.c::cdx_ipsec_build_shared_descriptor](cdx/cdx_dpa_ipsec.c#L716) plus a `share_mode` local. `HDR_SHARE_NEVER` is a fully-supported CAAM mode — each DECO works on a private copy of the descriptor — so SEC firmware can't race on per-SA GHASH state. **Result**: 86 % loss → 0.087 % loss. 0 ICV failures. TCP 4.98 Gbit/s (faster than CBC's 2.60 Gbit/s, makes sense — GCM is single-pass). CBC sanity-check passes unchanged at 2.60 Gbit/s — the gate is structural so non-GCM ciphers are unaffected.

  **A24a follow-up — wire-seq uniqueness**: a side effect of `HDR_SHARE_NEVER` is that each DECO maintains its own ESP sequence-number counter, so wire seqs collide across DECOs (106 / 500 = ~21 %). For our test setup (Vision `replay-window=0`) duplicates are accepted — but a production peer with anti-replay enabled (default `replay-window=32` for strongSwan-installed SAs) would reject every duplicate as a replay attack, reintroducing packet loss. The complete fix needs to cooperate with seq-counter atomicity — options: (i) configure the per-SA SEC FQ with single-DECO scheduling (so all ops go through one DECO and seqs are naturally unique), (ii) pre-assign the seq number from cdx software per packet before enqueue (much larger change, mirrors what upstream `caamalg_qi.c::cnstr_shdsc_rfc4106_encap` does), (iii) keep using the kernel's xfrm seq generator and have cdx fast-path consult it. **Until A24a lands, GCM is fine for any peer with anti-replay disabled, broken for any peer with it enabled**.

  **Earlier mis-diagnoses, for the record** (kept so future readers don't retread):
    - "Wedges under sustained large-frame load" — was actually MASQUERADE'd plain-IP forwarding bypassing the tunnel, with cumulative state from prior misconfigured runs. Test wasn't carrying ESP at all.
    - "Software xfrm fallback because cdx classifier never installs" — false; the classifier IS installed and the hardware path IS engaged. The 2 Mbit/s was GCM in hardware with 86 % ICV-fail TCP-collapsing the cwnd, not software xfrm.
    - "GHASH race in SEC firmware" — directionally right (cross-DECO race on GHASH state) but wrong about agency: it isn't a SEC firmware bug, it's cdx asking SEC to parallelise something that SEC firmware doesn't (and shouldn't have to) make atomic across DECOs. The user's "don't blame silicon" instinct was correct.
    - Two failed fix iterations (clearing PDBOPTS_ESP_IVSRC for GCM; seeding `pdb_en.gcm.iv = 1`) both wedged the DUT — SEC apparently treats a non-zero GCM `iv` field as "caller IV" which conflicts with IVSRC, and clearing IVSRC alone enters an unsupported state. Both reverted.

  **A24b code edits remain in tree** (cdx/dpa_ipsec.c ERN callback recycles buffers, qman_oos_fq failure leaks sainfo to avoid UAF, deferred-release timer capped at 30 s, sec_sa_context_free honors the release return). They're defensible on static-analysis terms (real defects in the recovery surface) and were not load-bearing for A24's actual bug, but they're worth keeping.

  **Severity**: was P0 (broke advertised AES-GCM); now P2 once strongSwan-default replay-window peers are added to the test mix (A24a). Production proposals can use **AES-128-GCM-16** for any peer that accepts replay-window=0 (plus **AES-128-CBC + HMAC-SHA-256** at 2.60 Gbit/s and **AES-128-CCM-16** at 1.45 Gbit/s as fallbacks). GMAC (rfc4543) remains broken at single-PPS — distinct bug class, separate investigation.

  **Reproduction runbook**: [IPSEC.md §3](IPSEC.md). To re-trigger the original 86 % ICV-fail symptom, revert [cdx/cdx_dpa_ipsec.c::cdx_ipsec_build_shared_descriptor](cdx/cdx_dpa_ipsec.c#L716)'s share-mode gate (force HDR_SHARE_SERIAL for GCM). Phase A printks gated on `CDX_DEBUG_GCM` are still in [cdx/control_ipsec.c](cdx/control_ipsec.c) + [cdx/cdx_dpa_ipsec.c](cdx/cdx_dpa_ipsec.c) — they served their diagnostic purpose; can be dropped now.
