...
- [ ] **A9. Tunnel offload — premise inverted by on-DUT tracing (2026-08-09): RX decap WORKS; it's TX encap that never offloads.**
  ...
  **Next probe:** per-entry `packet_count` (u64 at entry+256, stats enabled) read back after traffic — nonzero means the entry MATCHES and the punt happens later in the chain (points at PREEMPTIVE_CHECKS/INSERT_L3_HDR params); zero means the lookup itself misses. Either way the remaining fix is ucode-behavior territory — same NXP-support neighborhood as A24/DNCPE — unless the preemptive-check params turn out tunable from cdx.
